...
# --preFitValue=1.
FIT_OPTS = "--robustFit=1 --points=30  --setRobustFitAlgo=Minuit2 --setRobustFitStrategy=2 --setRobustFitTolerance=0.001"
...
if args.option == '7': #generate the datacards and do the fit
    ...
    os.system("combineCards.py --prefix=output/output_%s/ztt_mt_m_vis-  DM0_pt1=DM0_pt1%s.txt DM0_pt2=DM0_pt2%s.txt DM0_pt3=DM0_pt3%s.txt \
                ...
                >output/output_%s/combinecards.txt" % (args.era, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL,LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, LABEL, args.era))
    os.system("text2workspace.py output/output_%s/combinecards.txt" % (args.era))
    #for each decay mode
    for r in ["pt1","pt2","pt3","pt4","pt5","pt6","pt7"]: #["DM0","DM1","DM10","DM11"]
        for dm in ["DM0","DM1","DM10","DM11"]:
            ...
            WORKSPACE = "output/output_"+args.era+"/ztt_"+BINLABEL+".root"
            print("Region : "+r)
...
if args.option == '5': #generate the datacards and do the fit
    os.system("./TauES_ID/harvestDatacards_TES_idSF_bin_pt.py -y %s -c %s -e %s " %(args.era, args.config, EXTRATAG))  # Generating the datacards for option 5 
    ...
    for r in ["pt1","pt2","pt3","pt4","pt5","pt6","pt7"]: #["DM0","DM1","DM10","DM11"]
        ...
        WORKSPACE = "output/output_"+args.era+"/combinecards.root"
        os.system("combine -M MultiDimFit %s %s %s -n .%s %s %s %s --saveNLL \
                --saveSpecifiedNuis all" %(WORKSPACE, ALGO, POI_OPTS, BINLABEL, FIT_OPTS, XRTD_OPTS, CMIN_OPTS))

if args.option == '6': #generate the datacards and do the fit
    os.system("./TauES_ID/harvestDatacards_TES_idSF_bin_pt.py -y %s -c %s -e %s " %(args.era, args.config, EXTRATAG))  # Generating the datacards for option 5 
    ...
    for r in ["DM0","DM1","DM10","DM11"]: #
        ...
        POI_OPTS = "-P tes_%s  --setParameterRanges tid_SF_pt1=0.7,1.2:tid_SF_pt2=0.7,1.2:tid_SF_pt3=0.7,1.2:tid_SF_pt4=0.7,1.2:tid_SF_pt5=0.7,1.2:tid_SF_pt6=0.7,1.2:tid_SF_pt7=0.7,1.2:tes_DM0=%s:tes_DM1=%s:tes_DM10=%s:tes_DM11=%s \
                ...
                --freezeParameters r" %(r, RANGE, RANGE, RANGE, RANGE)
        WORKSPACE = "output/output_"+args.era+"/combinecards.root"
        os.system("combine -M MultiDimFit --fastScan %s %s %s -n .%s %s %s %s --saveNLL \
                --saveSpecifiedNuis all" %(WORKSPACE, ALGO, POI_OPTS, BINLABEL, FIT_OPTS, XRTD_OPTS, CMIN_OPTS))
        #os.system("python plot1D_Scan.py output/output_UL2018/higgsCombine.mt_"+v+"-"+r+setup["tag"]+"_DeepTau-UL2018-13TeV.MultiDimFit.mH90.root --y-cut 10 --y-max 10 --output plots/plots_"+args.era+"/plot1D_tes_%s --POI tes_%s  --main-label  tid_SF_%s --logo '' --logo-sub '' "%(r,r,r))
    ...
    for v in setup["observables"]:
        ...
        for r in variable["fitRegions"]:
            ...
            if args.option == '1':  # fit of tes_DM
                ...
                os.system("text2workspace.py output/output_%s/ztt_%s.txt" %(args.era, BINLABEL))
            ...
            if args.option == '5':  # simultaneous fit in DM and pt bins
                listregion = r.split("_")
                DM = listregion[0]
                print(">>>>>>> simultaneous fit of tes_" +
                    r + " in pt bins and tes_"+r + "in DM")
                POI_OPTS = "-P tes_%s  --setParameterRanges tid_SF_pt1=0.7,1.2:tid_SF_pt2=0.7,1.2:tid_SF_pt3=0.7,1.2:tid_SF_pt4=0.7,1.2:tid_SF_pt5=0.7,1.2:tid_SF_pt6=0.7,1.2:tid_SF_pt7=0.7,1.2:tes_DM0=%s:tes_DM1=%s:tes_DM10=%s:tes_DM11=%s \
                        -m 90 --setParameters r=1,tes_%s=1 --freezeParameters r " %(DM, RANGE, RANGE, RANGE, RANGE, DM)
                WORKSPACE = "output/output_"+args.era+"/combinecards.root"
                os.system("combine -M MultiDimFit -t -1 %s %s %s -n .%s %s %s %s --saveNLL \
                        --saveSpecifiedNuis all" %(WORKSPACE, ALGO, POI_OPTS, BINLABEL, FIT_OPTS, XRTD_OPTS, CMIN_OPTS))
                os.system("python plot1D_Scan.py output/output_"+args.era+"/higgsCombine.mt_"+v+"-"+r +setup["tag"]+"_DeepTau-UL2018-13TeV.MultiDimFit.mH90.root \
                        --output plots/plots_"+args.era+"/plot1D_tid_SF_%s --POI tes_%s  --main-label  tes_%s --logo '' --logo-sub '' " % (r, DM, DM))
...
if args.option == '2' :
    os.system("./TauES_ID/plotParabola_POI.py -p tid_SF -y %s -e %s -r %s,%s -s -a -c %s"% (args.era, EXTRATAG, min(setup["TESvariations"]["values"]), max(setup["TESvariations"]["values"]), args.config))
