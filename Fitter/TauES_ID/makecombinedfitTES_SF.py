...
def run_combined_fit(setup, setup_mumu, option, **kwargs):
    ...
    tid_SF_range = kwargs.get('tid_SF_range', "0.7,1.1")
    extratag     = kwargs.get('extratag',     "_DeepTau")
    algo         = kwargs.get('algo',         "--algo=grid --alignEdges=1  ")
    npts_fit     = kwargs.get('npts_fit',     "--points=81") ## 66 57 ## 66 for preEE 101 point, postEE 81 ## 31
    fit_opts     = kwargs.get('fit_opts',     "--robustFit=1 --setRobustFitAlgo=Minuit2 --setRobustFitStrategy=1 --setRobustFitTolerance=0.001 %s" %(npts_fit))
    xrtd_opts    = kwargs.get('xrtd_opts',    "--X-rtd FITTER_NEW_CROSSING_ALGO --X-rtd MINIMIZER_analyticMinimization=1 --X-rtd MINIMIZER_boundedPosDef=1")
    ...
    for r in setup["observables"]["m_vis"]["scanRegions"]:
        ...
        # Fit of tid_SF_DM by DM with tes as a nuisance parameter
            ...
            POI = "tid_SF_%s" % (r)
            NP = "rgx{.*tid.*}" 
            tid_SF_range = "0.8,1.1"
            print(">>>>>>> Scan of "+POI)
            if r == "DM0": 
                tes_range = "0.960,0.990"
            elif r == "DM1":
                tes_range = "0.980,1.000"
            elif r == "DM10":
                tes_range = "0.980,1.000"
            elif r == "DM11":
                tes_range = "0.980,1.000"
        ...
        # 2D Fit of tes_DM and tid_SF_DM by DM, both are pois
            ...
            MultiDimFit_opts = " -m 90 %s %s %s -n .%s %s %s %s %s --trackParameters rgx{.*tid.*},rgx{.*W.*},rgx{.*dy.*} --saveInactivePOI=1" %(workspace, algo, POI_OPTS, BINLABELoutput, fit_opts, xrtd_opts, cmin_opts, save_opts)
            os.system("combine -M MultiDimFit  %s " %(MultiDimFit_opts))
...
def plotScan(setup, setup_mumu, option, **kwargs):
    ...
    if option == '2' or option == '4'  :
        print(">>> Plot parabola")
        print("./TauES_ID/plotParabola_POI_region.py -p tid_SF -y %s -e %s  -s -a -c %s"% (era, extratag, config))
