...
for v in setup["observables"]:
    ...
    for r in variable["fitRegions"]:
        ...
        POI_OPTS="-P tes_%s --setParameterRanges tes_%s=%s -m 90 --setParameters r=1,tes_%s=1 --freezeParameters r " %(r,r,RANGE,r)
        ...
        # ##Impact
        os.system("combineTool.py -M Impacts -t -1 -n %s -d %s %s  --redefineSignalPOIs tes_%s %s %s %s --doInitialFit"%(BINLABEL, WORKSPACE, FIT_OPTS,r, POI_OPTS, XRTD_OPTS, CMIN_OPTS))
        os.system("combineTool.py -M Impacts -t -1 -n %s -d %s %s  --redefineSignalPOIs tes_%s %s %s %s --doFits --parallel 4"%(BINLABEL, WORKSPACE, FIT_OPTS,r, POI_OPTS, XRTD_OPTS, CMIN_OPTS))
        os.system("combineTool.py -M Impacts -t -1 -n %s -d %s %s  --redefineSignalPOIs tes_%s %s %s %s -o postfit/impacts_%s.json"%(BINLABEL, WORKSPACE, FIT_OPTS,r, POI_OPTS, XRTD_OPTS, CMIN_OPTS, BINLABEL))
