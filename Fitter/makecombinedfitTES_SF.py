...
parser.add_argument('-c', '--config', dest='config', type=str, default='TauES/config/defaultFitSetupTES_mutau.yml',
...
# --preFitValue=1.
FIT_OPTS = "--robustFit=1 --points=201  --setRobustFitAlgo=Minuit2 --setRobustFitStrategy=2 --setRobustFitTolerance=0.001"
...
if args.option == '7': #generate the datacards and do the fit
    os.system("./TauES/harvestDatacards_TES_idSF_bin_pt.py -y %s -c %s -e %s " %(args.era, args.config, EXTRATAG))  # Generating the datacards for option 5 
...
if args.option == '6': #generate the datacards and do the fit
    ...
    for r in ["DM0","DM1","DM10","DM11"]: #
        ...
        os.system("combine -M MultiDimFit --fastScan %s %s %s -n .%s %s %s %s --saveNLL \
                --saveSpecifiedNuis all" %(WORKSPACE, ALGO, POI_OPTS, BINLABEL, FIT_OPTS, XRTD_OPTS, CMIN_OPTS))
        #os.system("python plot1D_Scan.py ./output_UL2018/higgsCombine.mt_"+v+"-"+r+setup["tag"]+"_DeepTau-UL2018-13TeV.MultiDimFit.mH90.root --y-cut 10 --y-max 10 --output ./plots_"+args.era+"/plot1D_tes_%s --POI tes_%s  --main-label  tid_SF_%s --logo '' --logo-sub '' "%(r,r,r))
        ...
        for r in variable["fitRegions"]:
            ...
            if args.option == '5':  # simultaneous fit in DM and pt bins
                ...
                os.system("combine -M MultiDimFit -t -1 %s %s %s -n .%s %s %s %s --saveNLL \
                        --saveSpecifiedNuis all" %(WORKSPACE, ALGO, POI_OPTS, BINLABEL, FIT_OPTS, XRTD_OPTS, CMIN_OPTS))
                os.system("python plot1D_Scan.py output_"+args.era+"/higgsCombine.mt_"+v+"-"+r +setup["tag"]+"_DeepTau-UL2018-13TeV.MultiDimFit.mH90.root \
                        --output plots_"+args.era+"/plot1D_tid_SF_%s --POI tes_%s  --main-label  tes_%s --logo '' --logo-sub '' " % (r, DM, DM))
...
if args.option == '6' :
    os.system("./TauES/plotParabola_POI_region.py -p tes -y %s -e %s -r %s,%s -s -a -c %s"% (args.era, EXTRATAG, min(setup["TESvariations"]["values"]), max(setup["TESvariations"]["values"]), args.config))

if args.option == '5' :
    os.system("./TauES/plotParabola_POI_region.py -p tid_SF -y %s -e %s -r %s,%s -s -a -c %s"% (args.era, EXTRATAG, min(setup["TESvariations"]["values"]), max(setup["TESvariations"]["values"]), args.config))
