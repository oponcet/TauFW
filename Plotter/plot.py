...
def plot(sampleset,setup,parallel=True,tag="",extratext="",outdir="plots",era="",
  ...
  for selection in selections:
    ...
    for stack, variable in stacks.iteritems():
      ...
      stack.drawtext(text)
      stack.saveas(fname,ext='png',tag=tag)
      stack.close()
...
def main(args):
  ...
  for config in configs:
    ...
    for era in eras:
      ...
      sampleset = getsampleset(setup['channel'],era,fname=fname,rmsf=rmsfs,addsf=addsfs,split=split, configfile ="config_UL2018v10.json") #config_zpt.json
